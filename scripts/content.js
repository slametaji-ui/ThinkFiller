...
(function () {
  ...
  chrome.runtime.onMessage.addListener((request, sender, sendResponse) => {
    if (request.action === 'ping') {
      ...
      fillFormFields(request.data, request.fields).then(results => {
  ...
  function scrapeActiveForm() {
    ...
    elements.forEach((element) => {
      ...
      const isVisible = (rect.width > 0 && rect.height > 0 && window.getComputedStyle(element).display !== 'none') || tagName === 'select';
      if (!isVisible) return;
   ...
   */
  async function fillFormFields(data, fieldsMetadata = null) {
    const filled = [];
    const errors = [];

    // If metadata is provided, resolve elements dynamically to survive framework re-renders.
    if (fieldsMetadata && fieldsMetadata.length > 0) {
      for (const field of fieldsMetadata) {
        const thinkFillerId = field.thinkFillerId;
        if (!(thinkFillerId in data)) {
          continue;
        }
        ...
        if (thinkFillerId.startsWith('tf_radio_group_')) {
          ...
          let radios = Array.from(document.querySelectorAll(`input[type="radio"][data-thinkfiller-radio-group-id="${thinkFillerId}"]`));
          if (radios.length === 0 && field.name) {
            radios = Array.from(document.querySelectorAll(`input[type="radio"][name="${field.name}"]`));
          }
          ...
          // Ensure radio group elements have the IDs
          radios.forEach(radio => {
            if (!radio.getAttribute('data-thinkfiller-radio-group-id')) {
              radio.setAttribute('data-thinkfiller-radio-group-id', thinkFillerId);
            }
          });
          ...
          // Standard elements: Dynamically find the active DOM element
          let element = document.querySelector(`[data-thinkfiller-id="${thinkFillerId}"]`);
          if (!element || !element.isConnected) {
            if (field.id) {
              element = document.getElementById(field.id);
            }
            if ((!element || !element.isConnected) && field.name) {
              element = document.querySelector(`${field.tagName}[name="${field.name}"]`);
            }
          }

          if (!element) {
            errors.push(`Element not found for ID: ${thinkFillerId}`);
            continue;
          }

          // Restore ID if replaced/re-rendered
          if (!element.getAttribute('data-thinkfiller-id')) {
            element.setAttribute('data-thinkfiller-id', thinkFillerId);
          }
          ...
          if (element.disabled && tagName !== 'select') {
            console.warn(`ThinkFiller AI: Skipping disabled field ${thinkFillerId}`);
            continue;
          }

          try {
            // If it is a select dropdown, check if it's disabled or empty, and wait/poll up to 3 seconds for it to update
            if (tagName === 'select') {
              const targetValStr = String(value).trim().toLowerCase();
              for (let retry = 0; retry < 30; retry++) {
                // Fetch the element dynamically on every retry to handle re-renders!
                element = document.querySelector(`[data-thinkfiller-id="${thinkFillerId}"]`);
                if (!element || !element.isConnected) {
                  if (field.id) element = document.getElementById(field.id);
                  if ((!element || !element.isConnected) && field.name) {
                    element = document.querySelector(`${field.tagName}[name="${field.name}"]`);
                  }
                }
                ...
                for (let i = 0; i < element.options.length; i++) {
                  ...
                  if (optVal === targetValStr || optText === targetValStr || 
                      (targetValStr && (optText.includes(targetValStr) || targetValStr.includes(optText)))) {
                    hasOption = true;
                    break;
                  }
                }

                // If target option is found and element is enabled, stop waiting
                if (!element.disabled && hasOption) {
                  break;
                }
              ...
              if (!element || element.disabled) {
                console.warn(`ThinkFiller AI: Skipping select field ${thinkFillerId} because it was not found or remained disabled.`);
                continue;
              }

              // Restore ID again if re-rendered
              if (!element.getAttribute('data-thinkfiller-id')) {
                element.setAttribute('data-thinkfiller-id', thinkFillerId);
              }
            }
      ...
      // Fallback: visual sort querying (old method if fieldsMetadata is omitted)
      const selector = 'input:not([type="hidden"]):not([type="submit"]):not([type="reset"]):not([type="button"]):not([type="image"]):not([type="file"]), textarea, select';
      let elements = Array.from(document.querySelectorAll(selector));
  ...
  function setElementValue(element, value) {
    ...
    if (tagName === 'textarea' || (tagName === 'input' && type !== 'checkbox' && type !== 'radio')) {
      ...
      if (element.multiple) {
        ...
        if (!matched && element.options.length > 0) {
          ...
          if (fallbackOpt) {
            ...
            try {
              const setter = Object.getOwnPropertyDescriptor(window.HTMLSelectElement.prototype, 'value').set;
              setter.call(element, fallbackOpt.value);
            } catch (e) {
      ...
      if (element.checked !== shouldBeChecked) {
        try {
          const setter = Object.getOwnPropertyDescriptor(window.HTMLInputElement.prototype, 'checked').set;
          setter.call(element, shouldBeChecked);
        } catch (e) {
